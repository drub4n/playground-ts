...
export const krapekar = (input: number): number => {
  const ascendingInput = parseInt(input.toString().split('').sort((a,b) => parseInt(a) - parseInt(b)).join(''));
  const descendingInput = parseInt(input.toString().padEnd(4, '0').split('').sort((a,b) => parseInt(b) - parseInt(a)).join(''));
